...
resolver = RefResolver(base_uri=f"{Path(__file__).parent.as_uri()}/", referrer=cast(Any, True))
...
def validate(value: JSONValue, schema: str):
    jsonschema.validate(instance=value, schema=get_schema(schema), resolver=resolver, format_checker=format_checker)
...
def validate_result(schema: Union[str, JSONObject]):
    ...
    will require the return value of `get_asset` to validate `syngenta_databus_shared/json_schemas/Asset.schema.json`."""
    ...
    def decorator(func: Callable[_P, _R], /) -> Callable[_P, _R]:
        ...
        def decorated(*args: _P.args, **kwargs: _P.kwargs) -> _R:
            ...
            jsonschema.validate(instance=result, schema=schema, resolver=resolver, format_checker=format_checker)
            return result
